...
class BlogPost(models.Model):
    ...
    schema_headline = models.CharField(
        max_length=110,
        blank=True,
        null=True,
    ...
    schema_image_alt = models.CharField(
        max_length=125,
        blank=True,
        null=True,
    ...
    def get_schema_headline(self):
        """Get schema headline with fallback to title (max 110 chars)"""
        if self.schema_headline:
            return self.schema_headline[:110]
        elif self.title:
            return self.title[:110]
        return ""

    def get_schema_description(self):
        """Get schema description with fallback to meta description, excerpt, or content"""
        if self.schema_description:
            ...
            # Strip HTML tags and get first 300 characters for schema
            plain_text = strip_tags(self.content)
            # Clean up extra whitespace
            plain_text = re.sub(r'\s+', ' ', plain_text).strip()
            if len(plain_text) > 300:
                # Find the last complete sentence within 300 characters
                truncated = plain_text[:300]
                last_period = truncated.rfind('.')
                if last_period > 200:  # Ensure we have a reasonable length
    ...
    def get_schema_image_url(self):
        """Get full URL for schema image"""
        if self.featured_image:
            ...
            # If it's a relative URL, build absolute URI
            ...
            if hasattr(settings, 'SITE_URL'):
                ...
                return f"https://www.vacationbna.com{url}"
        return None

    def get_schema_image_alt(self):
        """Get alt text for schema image"""
        if self.schema_image_alt:
            return self.schema_image_alt
        elif self.title:
            return f"Featured image for: {self.title}"
        return "Blog post featured image"

    def generate_json_ld_schema(self, request=None):
        ...
        if hasattr(settings, 'SITE_URL'):
            ...
            base_url = "https://www.vacationbna.com"
        
        # Build post URL
        post_url = f"{base_url}/blog/{self.slug}"
        ...
        schema_data = {
            ...
            "headline": self.get_schema_headline(),
            "description": self.get_schema_description(),
            "datePublished": self.created_at.strftime("%Y-%m-%dT%H:%M:%S%z") if self.created_at else None,
            "dateModified": self.updated_at.strftime("%Y-%m-%dT%H:%M:%S%z") if self.updated_at else None,
            "publisher": {
        ...
        image_url = self.get_schema_image_url()
        if image_url:
            schema_data["image"] = {
                "@type": "ImageObject",
                "url": image_url,
                "alt": self.get_schema_image_alt()
            }
        
        # Add author if available
        # You can extend this to include actual author information
        schema_data["author"] = {
            "@type": "Organization",
            "name": "Vacation BNA",
            "url": base_url
    ...
    def get_json_ld_script_tag(self, request=None):
        """Get the complete script tag with JSON-LD schema"""
        schema_json = self.generate_json_ld_schema(request)
